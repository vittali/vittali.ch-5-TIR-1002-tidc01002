...
#define IOT_DEBUG(...)    \
	{\
	UART_PRINT("[CS_AWS] DEBUG:   %s L#%d ", __func__, __LINE__);  \
...
#define FUNC_ENTRY    \
	{\
	UART_PRINT( "FUNC_ENTRY:   %s L#%d \n\r", __func__, __LINE__);  \
	}
#define FUNC_EXIT    \
	{\
	UART_PRINT( "FUNC_EXIT:   %s L#%d \n\r", __func__, __LINE__);  \
	}
#define FUNC_EXIT_RC(x)    \
	{\
	UART_PRINT( "FUNC_EXIT:   %s L#%d Return Code : %d \n\r", __func__, __LINE__, x);  \
	return x; \
	}
#else
#define FUNC_ENTRY
...
#define IOT_WARN(...)   \
	{ \
	UART_PRINT( "[CS_AWS] WARN:  %s L#%d ", __func__, __LINE__);  \
	UART_PRINT( __VA_ARGS__); \
	UART_PRINT( "\n\r"); \
	}
#else
...
#define IOT_ERROR(...)  \
	{ \
	UART_PRINT( "[CS_AWS] ERROR: %s L#%d ", __func__, __LINE__); \
	UART_PRINT( __VA_ARGS__); \
	UART_PRINT( "\n\r"); \
	}
#else
